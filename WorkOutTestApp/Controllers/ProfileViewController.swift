...
//    }()
    ...
    private func setupViews() {
        ...
        collectionView.register(ProfileCollectionViewCell.self, forCellWithReuseIdentifier: cellIdentifier)
...
//        view.addSubview(targetView)
    ...
    private func getWorkoutResults() {
        ...
        for name in nameArray {
            ...
            workoutArray = localRealm.objects(WorkoutModel.self)
                .filter(predicateName).sorted(byKeyPath: "workoutName")
            var result = 0
            var image: Data?
            workoutArray.forEach { model in
                if model.workoutReps == 0 {
                    result += model.workoutTimer
                    let (min, _) = result.convertSecond()
                    result = min
                } else {
            ...
            let resultModel = ResultWorkout(name: name, result: result, imageDate: image)
